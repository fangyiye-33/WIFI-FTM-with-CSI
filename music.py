...
USE_REAL_CSI = False
SHOW_PLOT = True
CSI_CSV_PATH = r"./2.76m_L=4.5m_2.csv"
...
def find_music_peaks(spectrum, delay_range, min_prominence_db=1.0, min_width=1, threshold_db=-35):
...
def filter_by_rtt_iqr(results, verbose=True):
    valid_results = [r for r in results if r['is_valid']]
    if len(valid_results) < 4:
        if verbose:
            print("RTT过滤: 有效帧数不足，跳过RTT过滤")
        return results

    rtt_values = np.array([r['rtt_ns'] for r in valid_results])
    ...
    if verbose:
        print(f"\nRTT四分位过滤:")
        print(f"  Q1={q1:.2f}ns, Q3={q3:.2f}ns, IQR={iqr:.2f}ns")
        print(f"  有效范围: [{lower_bound:.2f}ns, {upper_bound:.2f}ns]")
    ...
    for r in results:
        ...
        elif lower_bound <= r['rtt_ns'] <= upper_bound:
            filtered_results.append(r)
        else:
            rtt_removed_count += 1
            if verbose:
                print(f"  剔除帧 {r['frame']}: RTT={r['rtt_ns']:.2f}ns (超出范围)")

    if verbose:
        print(f"  RTT过滤: 剔除 {rtt_removed_count} 帧")

    return filtered_results


def filter_by_delay_outlier(results, verbose=True):
    valid_results = [r for r in results if r['is_valid']]
    if len(valid_results) < 4:
        if verbose:
            print("修正时延过滤: 有效帧数不足，跳过时延过滤")
        return results

    delay_values = np.array([r['correction_delay_ns'] for r in valid_results])
    median_delay = np.median(delay_values)
    mad = np.median(np.abs(delay_values - median_delay))

    if mad < 1e-6:
        mad = np.std(delay_values) * 0.6745

    threshold = 4.0 * mad / 0.6745
    lower_bound = median_delay - threshold
    upper_bound = median_delay + threshold

    if verbose:
        print(f"\n修正时延异常值过滤:")
        print(f"  中位数={median_delay:.2f}ns, MAD={mad:.2f}ns")
        print(f"  有效范围: [{lower_bound:.2f}ns, {upper_bound:.2f}ns]")

    filtered_results = []
    delay_removed_count = 0

    for r in results:
        if not r['is_valid']:
            filtered_results.append(r)
        elif lower_bound <= r['correction_delay_ns'] <= upper_bound:
            filtered_results.append(r)
        else:
            delay_removed_count += 1
            if verbose:
                print(f"  剔除帧 {r['frame']}: 修正时延={r['correction_delay_ns']:.2f}ns (异常偏离)")

    if verbose:
        print(f"  时延过滤: 剔除 {delay_removed_count} 帧")
...
if __name__ == '__main__':
    if USE_REAL_CSI:
        ...
        results = filter_by_rtt_iqr(results, verbose=True)
        results = filter_by_delay_outlier(results, verbose=True)
